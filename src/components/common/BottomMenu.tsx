...
const MENU_ITEMS = [
  ...
  { key: 'upcoming', label: 'Upcoming', Icon: UpcomingIcon, ActiveIcon: UpcomingIconActive, route: 'UpcomingScreen' },
  { key: 'refer', label: 'Refer', Icon: ReferIcon, ActiveIcon: ReferIconActive, route: 'ReferAndEarn' },
...
const BottomMenu = () => {
  ...
  const activeKey =
    MENU_ITEMS.find(item => item.route === route.name)?.key || 'home';
        ...
        {MENU_ITEMS.slice(0, 2).map(({ key, label, Icon, ActiveIcon, route }) => {
          ...
          return (
            <TouchableOpacity
              ...
              <IconComponent color={isActive ? ACTIVE_COLOR : INACTIVE_COLOR} width={20} height={20} />
              <Text
                className={`mt-1 text-[8px] ${isActive ? 'text-[#02AFFF]' : 'text-[#888]'}`}>
                {label}
              </Text>
            </TouchableOpacity>
          );
